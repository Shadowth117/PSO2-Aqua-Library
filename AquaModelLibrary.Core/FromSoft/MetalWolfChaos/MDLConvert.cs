...
namespace AquaModelLibrary.Core.FromSoft.MetalWolfChaos
{
    public static class MDLConvert
    {
        public static AquaObject ConvertMDL(byte[] file, out AquaNode aqn)
        {
            ...
            {
                return MDLToAqua(SoulsFile<MDL0>.Read(file), out aqn);
            }
        ...
        public static AquaObject MDLToAqua(MDL0 mdl, out AquaNode aqn)
        {
            ...
            for (int i = 0; i < model.Nodes.Count; i++)
            {
                ...
                if (bone.MeshesA.Count > 0)
                {
                    ...
                    AddMesh(aqp, model, usedMatIds, bone, vertSetAddition, model.VerticesA, bone.MeshesA, worldMats, i, MDL.VertexFormat.A);
                }

                //Mesh B
                if (bone.MeshesB.Count > 0)
                {
                    int vertSetAddition = model.VerticesA.Count;
                    AddMesh(aqp, model, usedMatIds, bone, vertSetAddition, model.VerticesB, bone.MeshesB, worldMats, i, MDL.VertexFormat.B);
                }

                //Mesh C
                if (bone.MeshesC.Count > 0)
                {
                    int vertSetAddition = model.VerticesA.Count + model.VerticesB.Count;
                    foreach (var faceSet in bone.MeshesC)
                    {
                        AddMesh(aqp, model, usedMatIds, bone, vertSetAddition, model.VerticesC, faceSet.Meshes, worldMats, faceSet.BoneIndices[0], MDL.VertexFormat.C, faceSet.BoneIndices);
                    }
                }
            }
        ...
        private static void AddMesh(AquaObject aqp, MDL0 model, Dictionary<int, int> usedMatIds, MDL0.Node bone, int vertSetAddition, List<MDL0.Vertex> vertices, List<MDL0.Mesh> faceSets, List<Matrix4x4> worldMats, int boneId, MDL.VertexFormat fmt, short[] boneIndices = null)
        {
            ...
            foreach (var faceSet in faceSets)
            {
                ...
                else
                {
                    ...
                    //genMat.diffuseRGBA = mdlMat.Unk20;
                    genMat.matName = $"Material_{0}";
                    genMat.texNames = new List<string>() { model.Textures[mdlMat.DiffuseMapIndex] };
                ...
                */
                ...
                for (int f = faceSet.StartIndex; f < faceSet.StartIndex + faceSet.IndexCount - 2; f++)
                {
                    ...
                    var vertNormSum2 = GetVertNormal(vertA) + GetVertNormal(vertB) + GetVertNormal(vertC);
                    ...
                    AddVert(faceVtxl, vertA, bone, worldMats, boneId, boneIndices);
                    AddVert(faceVtxl, vertB, bone, worldMats, boneId, boneIndices);
                    AddVert(faceVtxl, vertC, bone, worldMats, boneId, boneIndices);
        ...
        private static Vector3 GetVertNormal(MDL0.Vertex vert)
        {
            return vert.Normal;
        }
        ...
        private static void AddVert(VTXL faceVtxl, MDL0.Vertex vert, MDL0.Node bone, List<Matrix4x4> worldMats, int boneId, short[] boneIndices)
        {
            ...
            if (vert.PrimaryVertexWeight != 0 || vert.SecondaryVertexWeight != 0)
            {
                ...
                if(boneIndices != null)
                {
                    faceVtxl.vertWeightIndices.Add(new int[] { boneIndices[vert.GetPrimaryWeightIndex()], boneIndices[vert.GetSecondaryWeightIndex()], boneIndices[2], boneIndices[3] });
                } else
                {
                    faceVtxl.vertWeightIndices.Add(new int[] { boneId, bone.ParentIndex >= 0 ? bone.ParentIndex : 0, 0, 0 });
                }
        ...
        public static AquaObject MDLToAqua(MDL mdl, out AquaNode aqn)
        {
            ...
            for (int i = 0; i < model.Nodes.Count; i++)
            {
                ...
                if (bone.FacesetsC.Count > 0)
                {
                    ...
                    {
                        AddMesh(aqp, model, usedMatIds, bone, vertSetAddition, model.VerticesC, faceSet.Facesets, worldMats, i, MDL.VertexFormat.C, faceSet.BoneIndices);
                    }
                }
            ...
            foreach (var faceSet in faceSets)
            {
                ...
                */
                ...
                for (int f = faceSet.StartIndex; f < faceSet.StartIndex + faceSet.IndexCount - 2; f++)
                {
                    ...
                    var vertNormSum = vertA.Normal + vertB.Normal + vertC.Normal;
                    var vertNormSum2 = GetVertNormal(vertA, bone, worldMats, boneId, boneIndices) + GetVertNormal(vertB, bone, worldMats, boneId, boneIndices) + GetVertNormal(vertC, bone, worldMats, boneId, boneIndices);
                    var test0FNormal = MathExtras.GetFaceNormal(vertA.Position, vertB.Position, vertC.Position);
                    var test0 = MathExtras.Angle(test0FNormal, vertNormSum2);
                    var test1FNormal = MathExtras.GetFaceNormal(vertC.Position, vertB.Position, vertA.Position);
                    var test1 = MathExtras.Angle(test1FNormal, vertNormSum2);
        ...
        private static Vector3 GetVertNormal(MDL.Vertex vert, MDL.Node bone, List<Matrix4x4> worldMats, int boneId, short[] boneIndices)
        {
            ...
            if (vert.PrimaryWeightFlag != MDLWeightIndex.Index0)
            {
                if(boneIndices != null)
                {
                    boneId = boneIndices[vert.GetPrimaryWeightIndex()];
                } else
                {
