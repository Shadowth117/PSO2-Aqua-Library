...
namespace AquaModelLibrary.Data.Utility
{
    public unsafe class ReferenceGenerator
    {
        ...
        private static void GenerateUILists(string pso2_binDir, string outputDirectory, string imageDirectory)
        {
            ...
            for (int i = 0; i < 100000; i++)
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    actbgOut.Add(name + "," + hash + "," + iname + "," + ihash + "," + i64name + "," + i64hash);
                }
            }
            ...
            for (int i = 0; i < 100000; i++)
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    ldBoardOut.Add(name + "," + hash + "," + iname + "," + ihash + "," + i64name + "," + i64hash);
                    var image = GetFirstImageFromIce(path);
                    if (image != null)
                    {
            ...
            for (int i = 0; i < 100000; i++)
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    ldCardOut.Add(name + "," + hash + "," + iname + "," + ihash + "," + i64name + "," + i64hash);
                    var image = GetFirstImageFromIce(path);
                    if (image != null)
                    {
                        var imagePath = Path.Combine(outputImageLDDirectory, Path.ChangeExtension(Path.GetFileName(name), ".png"));
            ...
            for (int i = 0; i < 100000; i++)
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    ldSleeveOut.Add(name + "," + hash + "," + iname + "," + ihash + "," + i64name + "," + i64hash);
                    var image = GetFirstImageFromIce(path);
                    if (image != null)
                    {
                        var imagePath = Path.Combine(outputImageLDDirectory, Path.ChangeExtension(Path.GetFileName(name), ".png"));
            ...
            Dictionary<string, List<List<PSO2Text.TextPair>>> actorNameText, Dictionary<string, List<List<PSO2Text.TextPair>>> actorNameTextReboot, Dictionary<string, List<List<PSO2Text.TextPair>>> actorNameTextNPCReboot)
        {
            ...
            {
                if (str.Length > 7 || !int.TryParse(str.Substring(4), out int result))
                {
                    continue;
                }
        ...
        private static void DumpPaletteData(string outputDirectory, CharacterMakingIndex aquaCMX)
        {
            ...
            if (aquaCMX.colDict.Count > 0 || aquaCMX.legacyColDict.Count > 0)
            {
                ...
                foreach (int id in aquaCMX.colDict.Keys)
                {
                    ...
                    {
                        using (Bitmap image = new Bitmap(7, 6, 7 * 4, PixelFormat.Format32bppRgb, new IntPtr(ptr)))
                        {
                ...
                foreach (int id in aquaCMX.legacyColDict.Keys)
                {
                    ...
                    {
                        using (Bitmap image = new Bitmap(21, 6, 21 * 4, PixelFormat.Format32bppRgb, new IntPtr(ptr)))
                        {
            ...
            if (textByCat.ContainsKey(category))
            {
                for (int sub = 0; sub < textByCat[category].Count; sub++)
                {
                    ...
                    foreach (var pair in textByCat[category][sub])
                    {
                        ...
                        {
                            int.TryParse(pair.name.Substring(2), out id);
                        }
                        else
                        {
                            if (int.TryParse(pair.name, out id) == false)
                            {
        ...
        public static Dictionary<int, string> ParseFaceVariationLua(StreamReader streamReader)
        {
            ...
            while (streamReader.EndOfStream == false)
            {
                ...
                else if (line.Contains("crop_name"))
                {
                    ...
                    {
                        faceStrings.Add(int.Parse(line.Substring(7)), language);
                    }
                    language = null;
                }
            }
        ...
        {
            return GetFileHash(icon + innerwearIcon + GetIconGender(int.Parse(id)) + id + ".ice");
        }

        public static string GetBasewearIconString(string id)
        {
            return GetFileHash(icon + basewearIcon + GetIconGender(int.Parse(id)) + id + ".ice");
        }

        public static string GetSkinIconString(string id)
        {
            return GetFileHash(icon + skinIcon + GetIconGender(int.Parse(id)) + id + ".ice");
        }
        ...
        {
            return icon + innerwearIcon + GetIconGender(int.Parse(id)) + id + ".ice";
        }

        public static string GetBasewearIconStringUnhashed(string id)
        {
            return icon + basewearIcon + GetIconGender(int.Parse(id)) + id + ".ice";
        }

        public static string GetSkinIconStringUnhashed(string id)
        {
            return icon + skinIcon + GetIconGender(int.Parse(id)) + id + ".ice";
        }
        ...
        public static void GetCostumeOuterIconString(PartData part, string pso2_binDir, string finalId)
        {
            ...
            var iconOuterName = icon + outerwearIcon + GetIconGender(int.Parse(finalId)) + finalId + ".ice";
