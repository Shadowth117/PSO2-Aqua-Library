...
namespace AquaModelLibrary.Data.AM2.BorderBreakPS4
{
    ...
    public unsafe class E_OBJ
    {
        ...
        public E_OBJ(BufferedStreamReaderBE<MemoryStream> streamReader)
        {
            ...
            //Read names
            streamReader.Seek(header.namePointerArrayOffset, SeekOrigin.Begin);
            ...
            //Read models
            streamReader.Seek(header.meshPointerArrayOffset, SeekOrigin.Begin);
            for (int i = 0; i < header.modelCount; i++)
            {
            ...
            {
                streamReader.Seek(modelOffs, SeekOrigin.Begin);
                ...
                foreach (var mesh in model.meshes)
                {
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertPositionsOffset, SeekOrigin.Begin);
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertNormalsOffset, SeekOrigin.Begin);
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertUVsOffset, SeekOrigin.Begin);
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertUV2sOffset, SeekOrigin.Begin);
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertWeightsOffset, SeekOrigin.Begin);
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                    ...
                    if (mesh.header.vertBoneIndicesOffset > 0)
                    {
                        ...
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                            ...
                            for (int i = 0; i < mesh.vertWeightIndices[v].Length; i++)
                            {
                                ...
                                {
                                    model.highestBone = mesh.vertWeightIndices[v][i];
                                }
                            }
                            streamReader.Seek(4, SeekOrigin.Current);
                        }
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.vertRigidWeightIndicesOffset, SeekOrigin.Begin);
                        for (int v = 0; v < mesh.header.vertCount; v++)
                        {
                            mesh.vertRigidWeightIndices.Add(streamReader.Read<Half>());
                            if ((float)mesh.vertRigidWeightIndices[v] > model.highestBone)
                            {
                    ...
                    {
                        streamReader.Seek(modelOffs + mesh.header.eobjMeshSubStructOffset, SeekOrigin.Begin);
                        ...
                        foreach (var ms in mesh.subStructs)
                        {
                            ...
                            streamReader.Seek(modelOffs + ms.faceIndicesOffset, SeekOrigin.Begin);
                            for (int f = 0; f < ms.faceIndicesCount; f++)
                            {
                                faceList.Add(streamReader.Read<ushort>());
                            }

                            streamReader.Seek(modelOffs + ms.bonePaletteOffset, SeekOrigin.Begin);
                ...
                //Read materials
                streamReader.Seek(modelOffs + model.header.materialOffset, SeekOrigin.Begin);
