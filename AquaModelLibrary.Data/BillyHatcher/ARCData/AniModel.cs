    ...
    public class AniModel : ARC
    {
        ...
        public override void Read(BufferedStreamReaderBE<MemoryStream> sr)
        {
            ...
            for (int i = 0; i < group1FileNames.Count; i++)
            {
                ...
                switch (fileName)
                {
                    //Technically this just points to the actual GJ model, but the file layout is still the same as modern ani_model, so we can go there even though it's not technically pointed to
                    case "MODEL_BODY":
        ...
        private void ReadAniModel(BufferedStreamReaderBE<MemoryStream> sr)
        {
            ...
            //In the trial there's a file that a null anim, but the only way to notice this is to check the next offset and see if it's the same
            int animSanityCheck = -1;
            if(boundingAddress != 0)
            {
                sr.Seek(0x20 + boundingAddress, SeekOrigin.Begin);
                animSanityCheck = sr.ReadBE<int>();
            }
            ...
            {
                if (animAddr != -1 && animAddr != animSanityCheck)
                {
